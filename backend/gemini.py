...
def _client():
    proxy = os.getenv("https_proxy") or os.getenv("HTTPS_PROXY") or "http://proxy.server:3128"
    return genai.Client(
...
def chat(messages: list[dict]) -> str:
    ...
    try:
        ...
        if not contents: raise ValueError("No message supplied")
        response=client.models.generate_content(
            model=MODEL,
            contents=contents,
            config=types.GenerateContentConfig(system_instruction=SYSTEM_INSTRUCTION,temperature=0.7,max_output_tokens=4096),
        )
        return (response.text or "").strip() or "لم أستطع توليد رد الآن. حاول مرة أخرى."
    finally:
        try: client.close()
