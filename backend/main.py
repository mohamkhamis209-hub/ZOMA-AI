...
async def analyze_image(
    ...
    prompt: str = Form("حلل الصورة واشرحها بالتفصيل وبالعربية."),
):
    content_type = (
        file.content_type
    ...
    if len(data) > MAX_IMAGE_BYTES:
        raise HTTPException(
            status_code=413,
            detail="حجم الصورة أكبر من 10MB.",
        )
    ...
    try:
        ...
        response = client.models.generate_content(
            ...
            config=types.GenerateContentConfig(
                system_instruction=(
                    "أنت ZOMA AI. حلل الصور بشكل مفيد وآمن، "
                    "وأجب بلغة المستخدم. إذا كانت الصورة لسؤال دراسي "
                    "فاشرح الحل خطوة بخطوة."
                ),
            ),
        )
        return {
